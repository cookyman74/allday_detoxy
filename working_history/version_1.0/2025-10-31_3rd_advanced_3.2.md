...
**피드백 내용**:
1. ✅ **긍정적**: SCHEDULE 단계가 계획대로 추가됨, v6 필드가 정확히 반영됨, 단일 활성화 원칙이 잘 드러남
2. ⚠️ **미사용 파라미터**: `triggerType`, `onTriggerTypeChange`가 `LocationSettingsStep`에서 사용되지 않음
3. ℹ️ **ScheduleGroupViewModel**: `loadAllLinkedCounts` 관련 상태가 ScheduleGroupScreen에서 사용되지 않는 경고 (이미 3.1 리뷰에서 해결됨)
...
**설명**:
- `triggerType`과 `onTriggerTypeChange` 파라미터를 `LocationSettingsStep` 함수 시그니처에서 제거
- 호출부에서도 해당 파라미터 전달을 제거
- `triggerType` 상태 변수는 저장 로직에서 사용되므로 유지
- UI에서 노출하지 않는 파라미터를 제거하여 코드 명확성 향상
...
| 항목 | Before | After |
|------|--------|-------|
| 컴파일 경고 | ⚠️ 2개 (미사용 파라미터) | ✅ 0개 |
| LocationSettingsStep 파라미터 | 16개 | 14개 |
| 코드 명확성 | 보통 (불필요한 파라미터 존재) | 높음 (필요한 파라미터만) |

---

**작업 완료 시간**: 2025-10-31
**작업자**: AI Assistant
**검증 상태**: ✅ 완료 (빌드 성공, Linter 오류 없음, 경고 없음)
**리뷰 반영**: ✅ 완료 (미사용 파라미터 제거)
