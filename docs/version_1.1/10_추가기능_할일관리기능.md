...
> **🔄 6차 리뷰 반영**: 수정 범위를 NOT_COMPLETED 항목까지 확장

- 전체 할일 목록 보기 (스케줄별 그룹화)
- **미완료/미응답 항목 수정** (목표 포함)
- 주간 할일 미리 계획
- 할일 달성률 통계
...
| 상태 | 수정 가능 | 비고 |
|------|----------|------|
| NO_RESPONSE | ✅ | 일반 항목 미응답 |
| NOT_COMPLETED (isGoal=true) | ✅ | **목표 미완료 수정 가능** |
| NOT_COMPLETED (isGoal=false) | ✅ | 필수 할일 미완료 |
| COMPLETED | ❌ | 수정 불필요 (이미 완료) |
...
| 보호 장치 | 설명 |
|-----------|------|
| 세션 중 접근 제한 | 집중 세션 진행 중에는 할일 페이지 비활성화 |
| 확인 vs 관리 분리 | 확인은 오버레이, 관리만 페이지에서 |
| 주간 계획 유도 | 매일 접근 대신 주 1회 계획 수립 권장 |
