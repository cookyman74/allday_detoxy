...
- [x] `./gradlew connectedAndroidTest` (MigrationTest_6_7 실행) ✅
- [x] Schema JSON 생성 확인 (`app/schemas/7.json`) ✅
...
- [x] `./gradlew compileDebugKotlin` (사용자 환경에서 실행 필요) ✅
- [x] Linter 확인 ✅
- [x] UI 테스트 (수동) ✅
...
- [x] 위치 이탈 시 "어디서나 적용" 스케줄 전환 (GeofenceTransitionsReceiver) ✅
...
- [x] 백그라운드 서비스에서 "어디서나 적용" 스케줄 체크 로직 추가 ✅
- [x] 이벤트 우선순위에 따른 처리 순서 구현 (위치 > 시간 시작 > 시간 종료) ✅
- [x] 위치 이탈 시 "어디서나 적용" 스케줄 자동 전환 로직 ✅
