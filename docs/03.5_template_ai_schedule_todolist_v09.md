...
## Phase 0: 데이터 모델 확장 (Day 0, 선행 작업)

### 목적
**우선순위 규칙 구현을 위한 데이터 레이어 준비**: `updatedAt` 필드 추가 및 마이그레이션

> **Note**: 이 작업은 Phase 1 시작 전 완료되어야 합니다. 별도 PR로 진행 가능.

### 0.1 Schema 확장 (Day 0, 2-3시간)

**파일**: 
- `app/src/main/java/com/allday/detoxy/data/local/entity/*.kt`
...
```kotlin
@Entity(tableName = "location_based_auto_runs")
data class LocationBasedAutoRun(
    // ... 기존 필드
    val createdAt: Long = System.currentTimeMillis(),
    val updatedAt: Long = createdAt  // 🆕 추가
)
```
...
```kotlin
@Entity(tableName = "time_based_auto_runs")
...
val MIGRATION_6_7 = object : Migration(6, 7) {
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        database.execSQL(
            """
            ALTER TABLE location_based_auto_runs 
            ADD COLUMN updatedAt INTEGER NOT NULL DEFAULT ${System.currentTimeMillis()}
            """
        )
        ...
        database.execSQL(
            """
            ALTER TABLE time_based_auto_runs 
            ADD COLUMN updatedAt INTEGER NOT NULL DEFAULT ${System.currentTimeMillis()}
            """
        )
    }
...
fun provideDetoxyDatabase(@ApplicationContext context: Context): DetoxyDatabase {
    return Room.databaseBuilder(context, DetoxyDatabase::class.java, "detoxy.db")
        .addMigrations(
...
### 0.3 DAO/Repository 업데이트 (Day 0, 1-2시간)

**파일**:
- `app/src/main/java/com/allday/detoxy/data/local/dao/*.kt`
- `app/src/main/java/com/allday/detoxy/data/repository/*.kt`

- [x] **Insert/Update 시 updatedAt 자동 갱신** ✅
...
// Repository 레이어에서 처리
suspend fun updateScheduleGroup(group: ScheduleGroup) {
    dao.insert(group.copy(updatedAt = System.currentTimeMillis()))
}
```

- [x] **모든 Repository에 updateXxx 메서드 추가** ✅
  - `ScheduleGroupRepositoryImpl.updateScheduleGroup()`
  - `LocationBasedAutoRunRepositoryImpl.updateAutoRun()`
  - `TimeBasedAutoRunRepositoryImpl.updateAutoRun()`

### 0.4 MigrationTest 추가 (Day 0, 1시간)

**파일**: `app/src/androidTest/java/com/allday/detoxy/data/local/migration/MigrationTest_6_7.kt` (신규)
...
class MigrationTest_6_7 {
    ...
    @Test
    fun migrate6To7_addsUpdatedAtField() {
        // v6 데이터 준비
        val db = helper.createDatabase(TEST_DB, 6)
        db.execSQL("INSERT INTO schedule_groups (...) VALUES (...)")
        db.close()
        ...
        // updatedAt 필드 존재 확인
        val cursor = dbV7.query("SELECT updatedAt FROM schedule_groups")
        cursor.moveToFirst()
        assertThat(cursor.getLong(0)).isGreaterThan(0)
    }
}
```
...
- [ ] `./gradlew connectedAndroidTest` (MigrationTest_6_7 실행)
- [ ] Schema JSON 생성 확인 (`app/schemas/7.json`)

**작업 기록**: `working_history/2025-11-XX_3.5th_advanced_0.md` ✅

**예상 소요 시간**: 5-7시간 (1일 이내)
...
> **선행 작업**: Phase 0 완료 필수 (updatedAt 필드 추가) ✅
...
data class TimeSlot(
    ...
    val enabledDays: List<DayOfWeek> = DayOfWeek.values().toList()  // 🆕 추가 (기본: 매일)
) {
    /**
     * 포맷팅된 시작 시간 (예: "10:00")
     */
...
- [x] TimeSlotInputDialog Composable 생성 (~200줄) ✅
```kotlin
...
fun TimeSlotInputDialog(
    ...
    var enabledDays by remember { mutableStateOf(existingSlot?.enabledDays ?: DayOfWeek.values().toList()) }  // 🆕 추가
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(
                onClick = {
                    onConfirm(TimeSlot(hour, minute, duration, preset, enabledDays))  // 🆕 enabledDays 추가
                },
...
- [x] TimeSlotItem Composable 생성 (~50줄) ✅
```kotlin
@Composable
fun TimeSlotItem(
    slot: TimeSlot,
...
- [x] QuickCreateScheduleDialog 대폭 수정 (~400줄) ✅
...
- [x] `createScheduleGroupWithTimeSlots()` 메서드 추가 ✅
        ...
        try {
            ...
            timeSlots.forEach { slot ->
                val autoRun = TimeBasedAutoRun(
                    ...
                    enabledDays = slot.enabledDays,  // 🆕 요일 설정 (필수)
...
- [x] QuickCreateScheduleDialog 호출 부분 수정 ✅
            ...
            scope.launch {
                val newGroupId = when (mode) {
                    CreationMode.TEMPLATE -> {
                        ...
                        scheduleViewModel.createFromTemplate(
                            name = scheduleName,
                            template = template
                        )
                    }
...
- [ ] `./gradlew compileDebugKotlin` (사용자 환경에서 실행 필요)
- [ ] Linter 확인
- [ ] UI 테스트 (수동)

**작업 기록**: `working_history/2025-11-02_03.5rd_location_time_scheduler_1.1.md` ✅
...
- [x] ScheduleGroupViewModel에 `createFromTemplate()` 추가 ✅
...
- [x] LocationConflictResolver 클래스 생성 (~300줄) ✅
...
class NonLocationScheduleManager @Inject constructor(
    ...
    suspend fun activateDefaultPolicy(): Result<Unit> = runCatching {
        ...
        if (nonLocationGroups.isEmpty()) {
            Log.d(TAG, "No applicable 'anywhere' schedule")
        ...
        if (applicableCandidates.isEmpty()) {
            Log.d(TAG, "No applicable 'anywhere' schedule for current time/day")
            return Result.success(Unit)
        }
        ...
        val selectedPolicy = if (applicableCandidates.size == 1) {
            ...
            Log.d(TAG, "Multiple applicable 'anywhere' schedules: ${applicableCandidates.size}")
...
- [ ] 백그라운드 서비스에서 "어디서나 적용" 스케줄 체크 로직 추가
- [ ] 이벤트 우선순위에 따른 처리 순서 구현 (위치 > 시간 시작 > 시간 종료)
- [ ] 위치 이탈 시 "어디서나 적용" 스케줄 자동 전환 로직
