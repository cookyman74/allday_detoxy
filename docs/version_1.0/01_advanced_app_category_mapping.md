...
- **목적**: 디지털 중독 진단 및 회복을 위한 카테고리별 앱 차단 설계
- **PRD 참조**: [01_advanced_prd.md §4.1 디톡시 제어 설정](./01_advanced_prd.md)

---

## 디톡시 제어 관점

### 중독 진단 및 회복 단계별 권장 사항
| 단계 | 위험 지수 | 권장 프리셋 | 차단 카테고리 |
|------|-----------|-------------|--------------|
| 고위험 (중독 진단) | 61-100 | 완전 차단 | SNS, Web, 영상, 기타 ON |
| 주의 (회복 초기) | 31-60 | 표준 디톡시 | SNS, 영상 ON |
| 회복 (유지 단계) | 0-30 | 완화 | 영상만 ON |

**핵심 원칙**:
- 메신저는 긴급 연락을 위해 기본 허용
- 사용자가 디톡시 강도를 단계적으로 조절 가능
- 루틴 연동: 특정 시간대(아침, 점심, 저녁)에 자동 차단 설정 가능
...
**중독 위험도**: 🔴 **높음** (무한 스크롤, 알림 중독)  
**회복 효과**: 차단 시 평균 1일 30분 이상 디톡시 시간 확보
...
**중독 위험도**: 🟡 **중간** (알림 중독, 대화 의존)  
**회복 효과**: 차단 시 평균 1일 20분 디톡시, 단 긴급 상황 고려 필요  
**특이사항**: 기본값 OFF (허용), 사용자가 완전한 디톡시를 원할 경우에만 차단 권장
...
**긴급 연락 허용 UX**:
1. 메신저 카테고리 차단 시 안내 다이얼로그 표시
2. 다이얼로그 내용:
   - "긴급 연락이 필요할 수 있어 메신저는 기본적으로 차단하지 않습니다."
   - "디지털 중독 회복을 위해 완전한 디톡시가 필요하다면 차단할 수 있습니다."
   - "⚠️ 긴급 연락이 불가능해질 수 있으니 신중히 선택하세요."
3. 버튼: "메신저도 차단" / "허용 유지 (권장)"
4. Analytics 로깅: `detoxy_settings_category_toggle(category="messenger", dialog_shown=true)`

**루틴 연동**:
- 루틴 시간대 (아침 9시, 점심 12시, 저녁 6시)에도 메신저 차단 여부를 별도 설정 가능
- 예: "점심 시간 루틴에서는 메신저도 차단" → 업무 집중 디톡시

---

### 3. Web 서핑
**목적**: 웹 브라우저 차단  
**중독 위험도**: 🔴 **높음** (정보 과다 소비, 뉴스 중독)  
**회복 효과**: 차단 시 평균 1일 40분 이상 디톡시 시간 확보
...
**중독 위험도**: 🔴 **매우 높음** (쇼츠 중독, 자동 재생)  
**회복 효과**: 차단 시 평균 1일 60분 이상 디톡시 시간 확보 (최대 효과)  
**우선순위**: 모든 프리셋에서 차단 권장 (완전 차단, 표준 디톡시, 완화 모두 포함)
...
### 5. 기타 앱 (Catch-All 카테고리)
**목적**: 명시되지 않은 모든 앱 차단 (게임 포함)  
**중독 위험도**: 🟡 **가변적** (앱에 따라 상이)  
**회복 효과**: 완전 차단 프리셋에서만 활성화 권장  
**확장 가능성**: 2차 고도화 시 게임/커머스 별도 카테고리화 예정

#### 5.1 게임 (향후 별도 카테고리화 예정)
**목적**: 모바일 게임 차단
...
### 1. 완전 차단 (Complete Block) - 고위험 사용자 권장
**목적**: 중독 진단 단계, 디지털 해독 집중  
**권장 대상**: 위험 지수 61-100, 초기 회복 단계

```kotlin
val PRESET_COMPLETE_BLOCK = setOf(
    AppCategory.SNS,          // ON - 무한 스크롤 차단
    AppCategory.WEB,          // ON - 정보 과다 소비 방지
    AppCategory.VIDEO_SHORTS, // ON - 쇼츠 중독 차단
    AppCategory.OTHER         // ON - 게임 등 기타 앱 차단
)
// MESSENGER: OFF (긴급 연락 고려)
```

**회복 효과**: 1일 평균 150분 이상 디톡시 시간 확보

---

### 2. 표준 디톡시 (Standard Detoxy) - 일반 사용자 권장
**목적**: 균형 잡힌 디지털 습관 형성, 회복 유지  
**권장 대상**: 위험 지수 31-60, 회복 중기 단계

```kotlin
val PRESET_STANDARD = setOf(
    AppCategory.SNS,          // ON - 주요 중독 원인 차단
    AppCategory.VIDEO_SHORTS  // ON - 쇼츠는 지속 차단
)
// WEB: OFF (업무/학습 검색 허용)
// MESSENGER: OFF (소통 허용)
// OTHER: OFF (일반 앱 허용)
...
**회복 효과**: 1일 평균 90분 디톡시 시간 확보

---

### 3. 완화 (Relaxed) - 저위험 유지 단계
**목적**: 디톡시 습관 유지, 재발 방지  
**권장 대상**: 위험 지수 0-30, 회복 완료 후 유지 단계

```kotlin
val PRESET_RELAXED = setOf(
    AppCategory.VIDEO_SHORTS  // ON - 쇼츠만 차단 (최소 개입)
)
// SNS: OFF (자율 사용)
// WEB: OFF
// MESSENGER: OFF
...
**회복 효과**: 1일 평균 60분 디톡시 시간 확보

---

### 4. 커스텀 (Custom)
**목적**: 사용자 맞춤 설정  
**Analytics 이벤트**: `preset="custom"`로 로깅

```kotlin
// 사용자가 프리셋을 선택한 후 카테고리를 수정하면 자동으로 "커스텀"으로 전환
if (modifiedCategories != currentPresetCategories) {
