...
- [x] **설정 단계** ✅
  ...
  - 타이머 시간 선택
  - 차단 프리셋 선택
  - 트리거 타입 선택 (도착 시 / 주기적) → UI에서 제외 (향후 구현)
...
- [2차 고도화 PRD](./02_advanced_autosetting_prd.md) - **2025-10-26 업데이트** 🆕
...
**상세 설계**: [PRD §4.4.6](./02_advanced_autosetting_prd.md#446-위치-기반-컨텍스트--시간-기반-스케줄-25차-고도화-)

**데이터 모델 변경**:
- `ScheduleGroup` 엔티티 추가 (v4 → v5 마이그레이션)
- `LocationBasedAutoRun`: `linkedScheduleGroupId` 필드 추가
- `TimeBasedAutoRun`: `scheduleGroupId`, `isIndependent` 필드 추가

**구현 우선순위**: 2.5차 고도화 (v0.7) - 선택적 구현
...
> **2.5차 고도화 방향**: 사용자 리뷰를 반영하여 "위치 기반 컨텍스트"와 "시간 기반 스케줄"의 복합 시나리오를 지원합니다. 회사/학교 등 특정 위치에서 자동으로 해당 시간표가 활성화되어, 반복적인 일상 루틴을 완벽하게 자동화합니다.
